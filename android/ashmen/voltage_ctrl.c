...
int find_ashmem_fd_in_proc(int pid, const char* name) {
    char fd_dir_path[64];
...
int get_pid_by_name(const char* name) {
    ...
    while ((entry = readdir(dir))) {
        ...
        if (fd >= 0) {
            ...
            close(fd);
            if (len > 0 && strstr(cmd, name)) return atoi(entry->d_name);
        }
    }
    closedir(dir);
    return -1;
...
int main(int argc, char **argv) {
    ...
    if (pid == -1) return 2;
    int remote_fd = find_ashmem_fd_in_proc(pid, "perf_bridge");
    if (remote_fd == -1) return 3;
    char final_path[64];
    snprintf(final_path, sizeof(final_path), "/proc/%d/fd/%d", pid, remote_fd);
    int fd = open(final_path, O_RDWR);
    if (fd < 0) return 4;

    ashmem_bridge_t *bridge = mmap(NULL, sizeof(ashmem_bridge_t), 
                                   PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    close(fd);
    if (bridge == MAP_FAILED) return 5;
    ...
    printf("[OK] Voltage injected via Atomic Release\n");
    munmap(bridge, sizeof(ashmem_bridge_t));
    return 0;
}
