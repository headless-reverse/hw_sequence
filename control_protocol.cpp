...
uint8_t calculate_crc(const ControlPacket& pkt) {
    const uint8_t* data_ptr = reinterpret_cast<const uint8_t*>(&pkt.magic);
    size_t len = offsetof(ControlPacket, crc) - offsetof(ControlPacket, magic);
    uint8_t crc = 0;
    for (size_t i = 0; i < len; ++i) {crc ^= data_ptr[i];}
    return crc;
}
...
bool validatePacket(const ControlPacket& pkt) {
    ...
    uint8_t expectedCrc = calculate_crc(pkt);
