...
void SequenceRunner::clearSequence() {m_commands.clear();emit logMessage("sequence queue cleared.", "#BDBDBD");}
...
bool SequenceRunner::startSequence() {
    ...
    emit logMessage("-- start sekwencji (hardware/ioctl) --", "#009688");
    executeNextCommand();
    return true;}
...
void SequenceRunner::executeNextCommand() {
    if (!m_isRunning || m_currentIndex >= m_commands.count()) {
        finishSequence(true);
        return;}
    const SequenceCmd &cmdRef = m_commands.at(m_currentIndex);
    emit commandExecuting(cmdRef.command, m_currentIndex + 1, m_commands.count());
    if (cmdRef.command.trimmed().toLower() == "wait") {
        int delay = qMax(0, cmdRef.delayAfterMs);
        m_delayTimer.stop();
        m_delayTimer.setInterval(delay);
        m_delayTimer.start();
        return;}
    if (cmdRef.isConditionalExecution) {
        emit logMessage(
            QString("[seq runner] executing conditional: %1").arg(cmdRef.command),
            "#FF9800"
        );
    }
    if (!m_executor) {
        qCritical() << "[seq runner] Error: Executor is NULL!";
        finishSequence(false);
        return;}
    m_isWaitingForMyCommand = true;
    m_executor->executeSequenceCommand(cmdRef.command, cmdRef.runMode);
}
...
void SequenceRunner::onCommandFinished(int exitCode, QProcess::ExitStatus status) {
    ...
    if (m_currentIndex >= m_commands.count())
        return;
    SequenceCmd currentCmd = m_commands.at(m_currentIndex);
    ...
    if (exitCode != 0 && currentCmd.stopOnError) {
        emit logMessage(QString("[seq runner] Critical:(kod %1).").arg(exitCode), "#F44336");
        finishSequence(false);
        return;}
    m_currentIndex++;
    if (currentCmd.isConditionalExecution) {
    ...
    if (m_currentIndex >= m_commands.count()) {
        ...
        if (currentCmd.delayAfterMs > 0) {
            m_delayTimer.start(currentCmd.delayAfterMs);
        } else {
            executeNextCommand();
        }
    }
...
void SequenceRunner::setIntervalToggle(bool toggle) {m_isInterval = toggle;
