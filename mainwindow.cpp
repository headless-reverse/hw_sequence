...
public:
    explicit LogDialog(QWidget *parent = nullptr) : QDialog(parent) {
        setWindowTitle("💉hw_injector Log Output");
        resize(900, 400);
		...
		connect(m_output, &QTextEdit::textChanged, this, [this]() {
...
MainWindow::MainWindow(QWidget *parent, const QString &adbPath, const QString &targetSerial)
    : QMainWindow(parent){
    setWindowTitle("💉 androd_injector");
    resize(1100, 720);
    setAcceptDrops(true);
    ensureJsonPathLocal();
	...
	HardwareController *hwCtrl = new HardwareController(this);
	hwCtrl->connectToDevice(targetSerial);
	m_executor->setHardwareController(hwCtrl);
...
// HW_RESIDENT
	...
	HardwareGrabbed* hwLogic = m_hardwareGrabbedWidget->hardwareLogic();
	connect(hwLogic, &HardwareGrabbed::logMessage, this, &MainWindow::appendLog);
    connect(hwLogic, &HardwareGrabbed::recordTapDetected, this, [this](int x, int y){
        if (m_swipeBuilder->isRecording()) {m_swipeBuilder->model()->addTap(x, y, 100, m_swipeBuilder->recordMode()); }});
    connect(hwLogic, &HardwareGrabbed::recordSwipeDetected, this, [this](int x1, int y1, int x2, int y2, int duration){
        if (m_swipeBuilder->isRecording()) {m_swipeBuilder->model()->addSwipe(x1, y1, x2, y2, duration, 100, m_swipeBuilder->recordMode());}});
    connect(hwLogic, &HardwareGrabbed::recordWaitDetected, this, [this](int ms){
        if (m_swipeBuilder->isRecording()) {m_swipeBuilder->model()->addWait(ms);}});
    connect(hwLogic, &HardwareGrabbed::recordKeyDetected, this, [this](int keyCode){
        if (m_swipeBuilder->isRecording()) {m_swipeBuilder->model()->addKey(QString::number(keyCode), 500, m_swipeBuilder->recordMode());}});
    connect(m_hardwareGrabbedWidget, &ViewHardwareGrabbed::hwKeyGenerated, this, [this](int linuxCode){
        if (m_hardwareGrabbedWidget->isRecordModeActive()) {
            if (m_swipeBuilder->model()) {m_swipeBuilder->model()->addKey(QString::number(linuxCode), 100, "ioctl");}}});
// --- PROCESS INJECTOR ---
    ViewProcessInjector *viewProcInjector = new ViewProcessInjector();
    m_processInjector = new ProcessInjector(m_hardwareGrabbedWidget->hardwareLogic(), viewProcInjector, this);
    m_dockProcessInjector = new QDockWidget(tr("process_injector"), this);
    m_dockProcessInjector->setObjectName("dockProcessInjector");
    m_dockProcessInjector->setWidget(viewProcInjector);
	addDockWidget(Qt::RightDockWidgetArea, m_dockProcessInjector);
	connect(m_hardwareGrabbedWidget->hardwareLogic(), &HardwareGrabbed::connectedToDevice, m_processInjector, &ProcessInjector::onDeviceConnected);
	connect(m_hardwareGrabbedWidget->hardwareLogic(), &HardwareGrabbed::disconnectedFromDevice, m_processInjector, &ProcessInjector::onDeviceDisconnected);
// --- CORE ---
	m_core = new Core(this); 
	CoreView *coreDock = new CoreView(m_core, this);
	m_dockCore = coreDock; // Przypisujemy do wskaźnika w klasie
	addDockWidget(Qt::LeftDockWidgetArea, m_dockCore);
// --- Menu Widoku ---
	...
	m_viewCoreAct = m_dockCore->toggleViewAction();
	m_viewCoreAct->setText("Core");
    m_viewProcessInjectorAct = m_dockProcessInjector->toggleViewAction();
    ...
    viewMenu->addAction(m_viewCoreAct);
    viewMenu->addAction(m_viewProcessInjectorAct);
// Detached Log Dialog
...
QWidget* MainWindow::createControlsWidget() {
    ...
    m_measureTimeControlsCheck->setToolTip("czas wykonania pojedynczej komendy");
...
void MainWindow::onScheduleButtonClicked() {
    ...
    appendLog(QString("Schedule in %1s: %2").arg(intervalSeconds).arg(cmdText), "#FFAA00");}
...
void MainWindow::runCommand() {
	...
	} else {m_executor->executeAdbCommand(cmd);
		if (cmd.startsWith("adb ") || cmd.startsWith("printf ") || cmd.contains("|")) {
			m_executor->executeAdbCommand(cmd);
		} else {
			m_executor->executeSequenceCommand(cmd, "adb");}}
    if (m_intervalToggle->isChecked()) {if (!m_commandTimer->isActive()) {
            int ms = m_intervalSpinBox->value();
            m_commandTimer->setInterval(ms);
            m_remainingMs = ms;
            m_commandTimer->start();
            appendLog(QString("       ⮚⮚ execution started ⮘⮘ %1 ms ").arg(ms), "#00BCD4");}
    } else {
        if (m_commandTimer->isActive()) {
            m_commandTimer->stop();
...
void MainWindow::showSettingsDialog() {
    ...
    dlg.setAdbPath(m_executor->adbPath());
    if (dlg.exec() == QDialog::Accepted) {
        QString newPath = dlg.adbPath();
        if (!newPath.isEmpty()) {
            m_executor->setAdbPath(newPath);
            if (m_videoClient) m_videoClient->setAdbPath(newPath);
            m_settings.setValue("adbPath", newPath);
            appendLog("ADB path updated: " + newPath, "#00BCD4");
        }
    }
}
...
void MainWindow::onDeviceSelected(int index) {
    ...
    if (!serial.isEmpty()) {
        ...
        if (m_swipeBuilder) {m_swipeBuilder->setAdbPath(m_executor->adbPath());}
    } else {m_executor->setTargetDevice("");}}
